...
void gua_apply_background_texture() {
    gua_out_color = texture2D(
    sampler2D(background_texture), gua_quad_coords).xyz;
}
...
void gua_apply_skymap_texture() {
    ...
    gua_out_color = textureGrad(sampler2D(background_texture), texcoord, dFdx(uv), dFdy(uv)).xyz;
}
