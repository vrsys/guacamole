...
bool abuf_insert(float depth)
{
  ...
  if (success) {
    ...
    imageAtomicMax(abuf_min_depth, ivec2(gl_FragCoord.xy)/2, pack_depth((1-depth)+0.00005));
    imageAtomicMax(abuf_max_depth, ivec2(gl_FragCoord.xy)/2, pack_depth(depth+0.00005));
  }
  return success;
}
