...
struct BoundingBox {
  ...
  BoundingBox()
      : min(std::numeric_limits<scalar_type>::max()),
        max(std::numeric_limits<scalar_type>::lowest()) {}

  /** Create a degenerate bounding box containing only a single point */
  explicit BoundingBox(point_type const& p) : min(p), max(p) {}

  explicit BoundingBox(point_type const& pmin, point_type const& pmax)
      : min(pmin), max(pmax) {}

  /** Create the smallest bounding box containing all the given points */
  explicit BoundingBox(std::vector<point_type> const& ps)
      : min(std::numeric_limits<scalar_type>::max()),
        max(std::numeric_limits<scalar_type>::lowest()) {
    if (!ps.empty()) {
  ...
  scalar_type size(unsigned dimension) const {
    return std::abs(max[dimension] - min[dimension]);
  }

  /** Lower and upper corners */
  std::pair<point_type, point_type> corners() const {
...
template <typename V>
BoundingBox<V> intersection(BoundingBox<V> const& lhs,
                            BoundingBox<V> const& rhs) {
  BoundingBox<V> tmp(lhs);
