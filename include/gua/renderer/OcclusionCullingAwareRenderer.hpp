...
class OcclusionCullingAwareRenderer {
	public:
		...
		virtual void render(Pipeline& pipe, PipelinePassDescription const& desc);
		void render_with_occlusion_culling(Pipeline& pipe, PipelinePassDescription const& desc);

	protected:
		virtual void renderOneNode(Pipeline& pipe, PipelinePassDescription const& desc, gua::node::Node*);
