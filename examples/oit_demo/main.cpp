...
int main(int argc, char** argv) {
  ...
  for(const auto& p : part_names) {
    ...
    mat->set_uniform("style", p.style)
     .set_uniform("opacity", 1.f)
     .set_uniform("roughness_map", std::string("data/roughness.jpg"))
     .set_show_back_faces(true);

    // derived from http://www.turbosquid.com/3d-models/speculate-3ds-free/302188
    // Royalty Free License
    auto part(loader.create_geometry_from_file(p.name,
                                               engine_directory + p.name + ".obj",
                                               mat,
  ...
  auto uniform_change = [](const std::shared_ptr<gua::Material>& m, const std::string& name, float delta) {
    ...
    if ((delta > 0 && h >= 1.f) || (delta < 0 && h <= 0.f)) return false;
  ...
  auto reset_all = [&]() {
    for (const auto& p: parts) {
      tasks[p.first] = [p, uniform_change]() {
        return uniform_change(p.second->get_material(), "cut_rad", -0.01f)
            || uniform_change(p.second->get_material(), "opacity", 0.01f)
  ...
  window->on_move_cursor.connect([&](gua::math::vec2 const& pos) {
    ...
    if (mode == Mode::Highlight) {
      ...
      std::set<gua::PickResult> picks = graph.ray_test(
          gua::Ray(get_translation(camera->get_world_transform()),
                   compute_ray(nx, ny)*ray_dist, 1.0),
      ...
      if (!picks.empty() && button_state == -1) {
        ...
        if (picked != selected_part) {
          if (selected_part) {
            tasks[selected_part->get_name()] = [selected_part, uniform_change]() {
              return uniform_change(selected_part->get_material(), "highlight", -0.01f);
            };
          }
          tasks[picked->get_name()] = [picked, uniform_change]() {
            return uniform_change(picked->get_material(), "highlight", 0.01f);
          };
  ...
  window->on_key_press.connect([&](int key, int scancode, int action, int mods) {
      ...
      if (key == 340) { // action mode (SHIFT)
        action_mode = (action != 0);

        if (mode == Mode::SeeThrough && action_mode) {
          apply_cut(get_translation(camera->get_world_transform()),
                    compute_ray(last_mouse_pos.x, last_mouse_pos.y));
        }
      }
  ...
  ticker.on_tick.connect([&]() {
    ...
    for (auto& t: tasks) {
      ...
      if (!result) erases.push_back(t.first);
    }
    for (auto& e: erases) tasks.erase(e);

    // animation
    for (const auto& p: parts) {
      const auto& ev = anim_events[p.first];
      if (ev.duration < 0.001) continue;
