...
vec3 gua_get_normal(vec2 frag_pos) {
    return texture2D(sampler2D(gua_gbuffer_normal), frag_pos).rgb;
}
...
void mainImage( out vec4 fragColor, out vec3 fragFlags, in vec2 fragCoord )
{
    vec2 p = fragCoord.xy;
    ...
    float green_to_red_ratio = col.g / col.r;
    float green_to_blue_ratio = col.g / col.b;
    if(green_to_red_ratio > 2.0 && green_to_blue_ratio > 6.0) {
      fragFlags = vec3(1.0, 1.0, 1.0);
    } else {
      fragFlags = vec3(0.0, 0.0, 0.0);
    }
...
void main() {
  vec4 color;
  vec3 flags;
 // mainImage(color, flags, gl_FragCoord.xy);
  gua_out_color = gua_get_normal();
  //gua_out_flags = flags;
  //gua_out_flags2 = vec3(0.0, flags.g, 0.0);
}
