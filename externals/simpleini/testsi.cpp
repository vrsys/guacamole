...
static void Test(CSimpleIni& ini)
{
...
#if defined(SI_SUPPORT_IOSTREAMS) && !defined(_UNICODE)
static bool TestStreams(const TCHAR* a_pszFile, bool a_bIsUtf8, bool a_bUseMultiKey, bool a_bUseMultiLine)
{
    // load the file
...
static bool TestFile(const TCHAR* a_pszFile, bool a_bIsUtf8, bool a_bUseMultiKey, bool a_bUseMultiLine)
{
    // load the file
    CSimpleIni ini(a_bIsUtf8, a_bUseMultiKey, a_bUseMultiLine);
    _tprintf(_T("Loading file: %s\n"), a_pszFile);
    ...
    if(fp)
    {
        ...
        convert.ConvertToStore(_T("; output from testsi.cpp test program") SI_NEWLINE SI_NEWLINE);
...
static bool ParseCommandLine(int argc, TCHAR* argv[], const TCHAR*& a_pszFile, bool& a_bIsUtf8, bool& a_bUseMultiKey, bool& a_bUseMultiLine)
{
    a_pszFile = 0;
    a_bIsUtf8 = false;
    ...
    if(!a_pszFile)
    {
        _tprintf(_T("Usage: testsi [-u] [-m] [-l] iniFile\n")
