...
namespace gua
{
ViveWindow::ViveWindow(std::string const& display) : GlfwWindow(), display_name_(display)
{
...
ViveWindow::~ViveWindow() { vr::VR_Shutdown(); }
...
void ViveWindow::calculate_viewing_setup()
{
    ...
    for(unsigned eye_num = 0; eye_num < 2; ++eye_num)
    {
        ...
        auto const& hmd_eye_projection = p_vr_system_->GetProjectionMatrix(eEye, near_distance, far_distance);
...
void ViveWindow::init_context()
{
    ...
    {
        p_vr_system_->GetRecommendedRenderTargetSize(&width, &height);
        left_texture_ = ctx_.render_device->create_texture_2d(scm::math::vec2ui(width, height), scm::gl::FORMAT_RGBA_8, 1, 1, 1);
        right_texture_ = ctx_.render_device->create_texture_2d(scm::math::vec2ui(width, height), scm::gl::FORMAT_RGBA_8, 1, 1, 1);
...
{
    if(DeviceID::CONTROLLER_0 != device_id && DeviceID::CONTROLLER_1 != device_id)
    {
...
math::vec2 const& ViveWindow::get_left_screen_size() const { return screen_size_[0]; }

math::vec2 const& ViveWindow::get_right_screen_size() const { return screen_size_[1]; }

math::vec3 const& ViveWindow::get_left_screen_translation() const { return screen_translation_[0]; }

math::vec3 const& ViveWindow::get_right_screen_translation() const { return screen_translation_[1]; }

void ViveWindow::start_frame() { GlfwWindow::start_frame(); }
...
void ViveWindow::display(scm::gl::texture_2d_ptr const& texture, bool is_left)
{
    ...
    glapi.glBlitFramebuffer(
        0, 0, tex_dimensions[0], tex_dimensions[1], config.left_position().x, config.left_position().y, config.left_resolution().x, config.left_resolution().y, GL_COLOR_BUFFER_BIT, GL_NEAREST);
...
void ViveWindow::trigger_haptic_pulse(unsigned int controller_id, float strength) const
{
