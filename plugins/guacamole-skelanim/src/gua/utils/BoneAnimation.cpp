...
BoneAnimation::BoneAnimation() : name { "default" }
, scalingKeys {}
, rotationKeys {}
, translationKeys {}
{}
...
, translationKeys {}
{
  ...
  for (unsigned i = start_frame; i <= end.GetFrameCount(FbxTime::eFrames30);
       ++i) {
    ...
    scalingKeys.push_back(Keyframe<scm::math::vec3f> {
      double(i - start_frame), to_gua::vec3f(Bone.EvaluateLocalScaling(time))
    });
      //get rotation from transformation matrix
    rotationKeys.push_back(Keyframe<scm::math::quatf> {
      double(i - start_frame), scm::math::quatf::from_matrix(transform_matrix)
    });
      //and translation
    translationKeys.push_back(Keyframe<scm::math::vec3f> {
      double(i - start_frame), scm::math::vec3f {
...
BoneAnimation::BoneAnimation(aiNodeAnim* anim) : name {
...
, translationKeys {}
{
  ...
  for (unsigned i = 0; i < anim->mNumScalingKeys; ++i) {
    scalingKeys.push_back(Keyframe<scm::math::vec3f> {
      anim->mScalingKeys[i].mTime, to_gua::vec3f(anim->mScalingKeys[i].mValue)
    });
  }
  for (unsigned i = 0; i < anim->mNumRotationKeys; ++i) {
    rotationKeys.push_back(Keyframe<scm::math::quatf> {
      anim->mRotationKeys[i].mTime, to_gua::quatf(anim->mRotationKeys[i].mValue)
    });
  }
  for (unsigned i = 0; i < anim->mNumPositionKeys; ++i) {
    translationKeys.push_back(Keyframe<scm::math::vec3f> {
      anim->mPositionKeys[i].mTime, to_gua::vec3f(anim->mPositionKeys[i].mValue)
    });
  }
}

BonePose BoneAnimation::calculate_pose(float time) const {
  return BonePose { calculate_value(time, scalingKeys),
                    calculate_value(time, rotationKeys),
                    calculate_value(time, translationKeys) };
}

std::string const& BoneAnimation::get_name() const { return name; }

scm::math::vec3f BoneAnimation::interpolate(scm::math::vec3f val1,
                                            scm::math::vec3f val2,
                                            float factor) const {
  return val1 * (1 - factor) + val2 * factor;
}

scm::math::quatf BoneAnimation::interpolate(scm::math::quatf val1,
                                            scm::math::quatf val2,
                                            float factor) const {
...
template <class T>
T BoneAnimation::calculate_value(float time,
                                 std::vector<Keyframe<T> > keys) const {
  if (keys.size() == 1) {
    return keys[0].value;
  }
