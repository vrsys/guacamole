...
SkeletalAnimationRenderer::SkeletalAnimationRenderer(RenderContext const& ctx)
    : bones_block_(ctx.render_device), last_frame_(0) {
#ifdef GUACAMOLE_RUNTIME_PROGRAM_COMPILATION
  ResourceFactory factory;
  std::string v_shader =
      factory.read_shader_file("resources/shaders/skinned_mesh_shader.vert");
  std::string f_shader =
      factory.read_shader_file("resources/shaders/tri_mesh_shader.frag");
#else
  std::string v_shader =
      Resources::lookup_shader("shaders/skinned_mesh_shader.vert");
  std::string f_shader =
      Resources::lookup_shader("shaders/tri_mesh_shader.frag");
#endif

  program_stages_.push_back(
      ShaderProgramStage(scm::gl::STAGE_VERTEX_SHADER, v_shader));
  program_stages_.push_back(
      ShaderProgramStage(scm::gl::STAGE_FRAGMENT_SHADER, f_shader));
}

////////////////////////////////////////////////////////////////////////////////

void SkeletalAnimationRenderer::create_state_objects(RenderContext const& ctx) {
  rs_cull_back_ = ctx.render_device
      ->create_rasterizer_state(scm::gl::FILL_SOLID, scm::gl::CULL_BACK);
  rs_cull_none_ = ctx.render_device
...
void SkeletalAnimationRenderer::render(Pipeline& pipe,
  ...
  if (sorted_objects != scene.nodes.end() &&
      sorted_objects->second.size() > 0) {
    ...
    std::string const gpu_query_name =
        "GPU: Camera uuid: " +
        std::to_string(pipe.current_viewstate().viewpoint_uuid) +
        " / SkeletalanimationPass";
    std::string const cpu_query_name =
        "CPU: Camera uuid: " +
        std::to_string(pipe.current_viewstate().viewpoint_uuid) +
        " / SkeletalanimationPass";
    ...
    for (auto const& object : sorted_objects->second) {
      ...
      auto skel_anim_node(
          reinterpret_cast<node::SkeletalAnimationNode*>(object));
      if (pipe.current_viewstate().shadow_mode &&
          skel_anim_node->get_shadow_mode() == ShadowMode::OFF) {
        continue;
      }
      ...
      for (unsigned i(0); i < materials.size(); ++i) {
        ...
        if (current_material != materials[i]->get_shader()) {
          ...
          if (current_shader) {
            ...
            current_shader->set_uniform(
                ctx, 1.0f / target.get_width(), "gua_texel_width");
            current_shader->set_uniform(
                ctx, 1.0f / target.get_height(), "gua_texel_height");
            // hack
            current_shader->set_uniform(
                ctx,
        ...
        if (current_shader && geometries[i]) {
          auto model_view_mat = scene.rendering_frustum.get_view() *
                                skel_anim_node->get_cached_world_transform();
          UniformValue normal_mat(
              math::mat4f(scm::math::transpose(scm::math::inverse(
                  skel_anim_node->get_cached_world_transform()))));

          int rendering_mode = pipe.current_viewstate().shadow_mode
                                   ? (skel_anim_node->get_shadow_mode() ==
                                              ShadowMode::HIGH_QUALITY
                                          ? 2
                                          : 1)
                                   : 0;
          ...
          current_shader->apply_uniform(
              ctx,
              "gua_model_matrix",
              math::mat4f(skel_anim_node->get_cached_world_transform()));
          current_shader->apply_uniform(
              ctx, "gua_model_view_matrix", math::mat4f(model_view_mat));
          current_shader->apply_uniform(ctx, "gua_normal_matrix", normal_mat);
          current_shader->apply_uniform(
              ctx, "gua_rendering_mode", rendering_mode);
          ...
          current_rasterizer_state =
              materials[i]->get_show_back_faces() ? rs_cull_none_
                                                  : rs_cull_back_;

          if (ctx.render_context->current_rasterizer_state() !=
              current_rasterizer_state) {
