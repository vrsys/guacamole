...
#include <gua/video3d/video3d_geometry/fastdxt/dxt.h>
...
word ColorTo565(const byte* color) { return ((color[0] >> 3) << 11) | ((color[1] >> 2) << 5) | (color[2] >> 3); }
...
int ColorDistance(const byte* c1, const byte* c2) { return ((c1[0] - c2[0]) * (c1[0] - c2[0])) + ((c1[1] - c2[1]) * (c1[1] - c2[1])) + ((c1[2] - c2[2]) * (c1[2] - c2[2])); }
...
int ColorLuminance(const byte* color) { return (color[0] + color[1] * 2 + color[2]); }
