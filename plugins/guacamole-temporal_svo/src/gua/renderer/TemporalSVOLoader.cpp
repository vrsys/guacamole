...
// class header
#include <gua/renderer/TemporalSVOLoader.hpp>
...
// static variables
/////////////////////////////////////////////////////////////////////////////
std::unordered_map<std::string, std::shared_ptr<::gua::node::Node>> TemporalSVOLoader::loaded_files_ = std::unordered_map<std::string, std::shared_ptr<::gua::node::Node>>();

/////////////////////////////////////////////////////////////////////////////
TemporalSVOLoader::TemporalSVOLoader() : _supported_file_extensions()
{
    _supported_file_extensions.insert("kttree");
}
...
/////////////////////////////////////////////////////////////////////////////
std::shared_ptr<node::Node> TemporalSVOLoader::load_geometry(std::string const& file_name, unsigned flags)
{
    ...
    else
    {
        ...
        {
            // cached_node = load(file_name, flags);
            GeometryDescription desc("TemporalSVO", file_name, 0, flags);

            std::shared_ptr<TemporalSVOResource> resource = nullptr;
            resource = std::make_shared<TemporalSVOResource>(file_name, flags & TemporalSVOLoader::MAKE_PICKABLE);
            
            GeometryDatabase::instance()->add(desc.unique_key(), resource);

            cached_node = std::make_shared<node::TemporalSVONode>("", desc.unique_key(), file_name);
            cached_node->update_cache();

            loaded_files_.insert(std::make_pair(key, cached_node));

            // normalize mesh position and rotation
            if(flags & TemporalSVOLoader::NORMALIZE_POSITION || flags & TemporalSVOLoader::NORMALIZE_SCALE)
            {
                auto bbox = cached_node->get_bounding_box();

                if(flags & TemporalSVOLoader::NORMALIZE_POSITION)
                {
                    auto center((bbox.min + bbox.max) * 0.5f);
                    cached_node->translate(-center);
                }

                if(flags & TemporalSVOLoader::NORMALIZE_SCALE)
                {
                    auto size(bbox.max - bbox.min);
                    auto max_size(std::max(std::max(size.x, size.y), size.z));
                    cached_node->scale(1.f / max_size);
...
std::shared_ptr<node::Node> TemporalSVOLoader::create_geometry_from_file(
    std::string const& node_name, std::string const& file_name, std::shared_ptr<Material> const& fallback_material, unsigned flags)
{
...
std::shared_ptr<node::Node> TemporalSVOLoader::create_geometry_from_file(std::string const& node_name, std::string const& file_name, unsigned flags)
{
    auto cached_node(load_geometry(file_name, flags));
...
void TemporalSVOLoader::apply_fallback_material(std::shared_ptr<node::Node> const& root, std::shared_ptr<Material> const& fallback_material, bool no_shared_materials)
{
    auto g_node(std::dynamic_pointer_cast<node::TemporalSVONode>(root));
...
/*
std::shared_ptr<node::TV_3Node> TemporalSVOLoader::load_geometry(std::string const& filename, unsigned flags,
  ...
  Logger::LOG_WARNING << "TemporalSVOLoader::load_tv_3() : unable to create Lod Node" << std::endl;
...
bool TemporalSVOLoader::is_supported(std::string const& file_name) const
{
...
/*
std::pair<std::string, math::vec3> TemporalSVOLoader::pick_lod_bvh(math::vec3 const& ray_origin,
...
std::set<PickResult> TemporalSVOLoader::pick_lod_interpolate(math::vec3 const& bundle_origin,
