...
namespace gua {
  ...
  NormalizationSubRenderer::NormalizationSubRenderer() : PLodSubRenderer(), fullscreen_quad_(nullptr) {
  	_load_shaders();
  }

  void NormalizationSubRenderer::create_gpu_resources(gua::RenderContext const& ctx,
...
#error "This works only with GUACAMOLE_RUNTIME_PROGRAM_COMPILATION enabled"
  ...
  void NormalizationSubRenderer::_upload_normalization_pass_uniforms(RenderContext const& ctx, 
                                                                     gua::plod_shared_resources& shared_resources) {
