...
namespace gua {
  ...
  LogToLinSubRenderer::LogToLinSubRenderer() : PLodSubRenderer(), 
                                               fullscreen_quad_(nullptr),
                                               nearest_sampler_state_(nullptr),
                                               no_depth_test_with_writing_depth_stencil_state_(nullptr)  {
  	_load_shaders();
  }

  void LogToLinSubRenderer::create_gpu_resources(gua::RenderContext const& ctx,
                                                 scm::math::vec2ui const& render_target_dims,
                                                 gua::plod_shared_resources& shared_resources) {
    // attachments
    resource_ptrs_.attachments_[plod_shared_resources::AttachmentID::DEPTH_PASS_LIN_DEPTH] = ctx.render_device
