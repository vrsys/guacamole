...
namespace gua {
  ...
  ShadowSubRenderer::ShadowSubRenderer() : PLodSubRenderer(){
  	_load_shaders();
  }

  void ShadowSubRenderer::create_gpu_resources(gua::RenderContext const& ctx,
  ...
  void ShadowSubRenderer::
  render_sub_pass(Pipeline& pipe, PipelinePassDescription const& desc,
  				  gua::plod_shared_resources& shared_texture_resources,
    ...
    if( !no_backface_culling_rasterizer_state_ ) {
  ...
  void ShadowSubRenderer::_load_shaders() {
...
#error "This works only with GUACAMOLE_RUNTIME_PROGRAM_COMPILATION enabled"
  ...
  void ShadowSubRenderer::_upload_model_dependent_uniforms(RenderContext const& ctx, node::PLodNode* plod_node, gua::Pipeline& pipe) {
