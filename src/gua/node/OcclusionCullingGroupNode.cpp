...
void OcclusionCullingGroupNode::build_tree_across_children(){
    ...
    if(get_children().size() > 2) {
        splitting_queue.push(copy());
    }

    std::cout<< "queue has size of " << splitting_queue.size()<<std::endl;
...
void cleanup_intermediate_nodes(std::shared_ptr<gua::node::Node> parent_node) {
...
void split_children(std::shared_ptr<gua::node::Node> parent_node, std::vector<std::shared_ptr<gua::node::Node>> & sorted_vector, 
    ...
    if(mode == true) {
        if (pivot == 1) {
            auto transform_node_R = parent_node->add_child<gua::node::TransformNode>("transform_node_R");
            parent_node->add_child(sorted_vector[0]);
            ...
            auto transform_node_L = parent_node->add_child<gua::node::TransformNode>("transform_node_L");
            parent_node->add_child(sorted_vector[pivot]);
            for(auto it = sorted_vector.begin(); it != sorted_vector.end(); ++it) {
                if(index < pivot) {
...
void OcclusionCullingGroupNode::split_children(std::shared_ptr<gua::node::Node> parent_node, std::vector<std::shared_ptr<gua::node::Node>> & sorted_vector, 
                    unsigned int candidate_index, unsigned int candidate_element_offset, bool mode) {
    parent_node->clear_children();
...
double OcclusionCullingGroupNode::calculate_cost(std::shared_ptr<gua::node::Node> node){
    node->update_cache();
