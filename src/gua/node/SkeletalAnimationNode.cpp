...
namespace node {
  ...
  ////////////////////////////////////////////////////////////////////////////////
  std::string const& SkeletalAnimationNode::get_filename() const {
    return filename_;
  }

  ////////////////////////////////////////////////////////////////////////////////
  void SkeletalAnimationNode::set_filename(std::string const& v) {
    filename_ = v;
  ...
  void SkeletalAnimationNode::update_cache() {
    ...
    if (filename_changed_)
    {
      if (filename_ != "")
      {
        ...
        geometry_ = std::dynamic_pointer_cast<SkeletalAnimationRessource>(GeometryDatabase::instance()->lookup(filename_));

        if (!geometry_) {
          Logger::LOG_WARNING << "Failed to get SkeletalAnimationRessource for " << filename_ << ": The data base entry is of wrong type!" << std::endl;
        }
      }
  ...
  std::shared_ptr<Node> SkeletalAnimationNode::copy() const {
    auto result(std::make_shared<SkeletalAnimationNode>(get_name(), filename_, material_, get_transform()));
