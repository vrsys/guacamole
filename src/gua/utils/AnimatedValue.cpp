...
AnimatedValue::AnimatedValue(Direction direction,
                             float start,
                             float end,
                             float duration,
                             float multiplier)
...
float AnimatedValue::updateLinear(float t, float s, float e) {
  return (s + t * (e - s));
}

float AnimatedValue::updateEaseIn(float t, float s, float e) {
  return (s + (t * t * ((multiplier_ + 1) * t - multiplier_)) * (e - s));
}

float AnimatedValue::updateEaseOut(float t, float s, float e) {
  return (s + ((t - 1) * (t - 1) * ((multiplier_ + 1) * (t - 1) + multiplier_) +
               1) * (e - s));
}
