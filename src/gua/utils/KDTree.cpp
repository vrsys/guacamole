...
void KDTree::ray_test(Ray const& ray, aiMesh* mesh, PickResult::Options const& options,
  ...
  if (root_) {
    ...
    if (options & PickResult::PICK_ONLY_FIRST_FACE &&
        options & PickResult::PICK_ONLY_FIRST_OBJECT) {
...
KDTree::KDNode* KDTree::build(
    std::vector<std::vector<LeafData> > const& sorted_triangles,
    ...
    // box
    float split_position(sorted_triangles[dim][sorted_triangles[dim].size() / 2]
                             .bbox_.min[dim]);
    ...
    for (unsigned i(0); i < 3; ++i) {
        for (unsigned j(0); j < sorted_triangles[i].size(); ++j) {
            ...
            } else if (sorted_triangles[i][j].bbox_.min[dim] >=
                       split_position) {
    ...
    } else if (right_list[dim].size() == sorted_triangles[dim].size() ||
               left_list[dim].size() == sorted_triangles[dim].size()) {
        right_child = new KDNode(right_list[dim]);
        left_child = new KDNode(left_list[dim]);
...
bool KDTree::intersect_one(KDNode* node,
            ...
            if (intersection < Ray::END) {
              if (hits.empty() || intersection < hits.begin()->distance) {
                  ...
                  if (options & PickResult::GET_TEXTURE_COORDS) {
                    tex_coords =
                      triangles[triangle.id_].get_texture_coords_interpolated(mesh, position);
                  }
    ...
    if (traversing_ray.t_max_ >= 0.f) {
        ...
        math::vec3 b(traversing_ray.origin_ +
                     traversing_ray.t_max_ * traversing_ray.direction_);

        // if ray is only to the left
        if (a[node->splitting_dimension_] <= node->splitting_position_ &&
            b[node->splitting_dimension_] <= node->splitting_position_) {
        ...
        } else if (a[node->splitting_dimension_] >= node->splitting_position_ &&
                   b[node->splitting_dimension_] >= node->splitting_position_) {
        ...
        // ray crosses splitting plane
            ...
            if (a[node->splitting_dimension_] < node->splitting_position_) {
                ...
                if (node->left_child_ &&
                    intersect_one(node->left_child_, ray, mesh, options, triangles, hits)) {
                    return true;
                }

                if (node->right_child_ &&
                    intersect_one(node->right_child_, ray, mesh, options, triangles, hits)) {
                    return true;
                }
...
bool KDTree::LeafData::Comparator::operator()(LeafData const & lhs,
                                              LeafData const & rhs) const {
    return lhs.bbox_.min[dim_] < rhs.bbox_.min[dim_];
}
...
KDTree::KDNode::KDNode(std::vector<LeafData> const & data)
    : data_(data),
      left_child_(nullptr),
...
KDTree::KDNode::KDNode(KDNode * left_child,
                       KDNode * right_child,
...
void KDTree::KDNode::print(unsigned depth) const {
    ...
    for (unsigned i(0); i < depth; ++i)
        msg += " ";
    msg += is_leaf_ ? string_utils::to_string(data_.size()) + " leaf(s): "
                    : "internal: ";

    if (is_leaf_)
      msg += "";
    else
      msg += "Split @ " + string_utils::to_string(splitting_position_) + " " +
