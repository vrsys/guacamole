...
Profiler::Timer::Timer(Profiler const& profiler, std::string const& name)
...
Profiler::Timer::Timer(Profiler const& profiler,
...
void Profiler::print() {
    ...
    if (profile_host_->enabled() && ++frame_count_ > interval_) {
        ...
        for (auto& name : timer_names_) {
            std::cout << name << ": "
                      << scm::gl::util::profiling_result(
                             profile_host_,
                             name,
                             scm::time::time_io(scm::time::time_io::msec, 3))
                      << std::endl;
        }
