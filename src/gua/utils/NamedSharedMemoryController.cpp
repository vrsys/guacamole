...
bool NamedSharedMemoryController::check_memory_segment_exists(
    std::string const& segment_name) const {
  auto memory_segment_it = mNamedMemorySegments.find(segment_name);
  return (mNamedMemorySegments.end() != memory_segment_it);
}

bool NamedSharedMemoryController::check_constructed_object_exists(
...
void NamedSharedMemoryController::add_memory_segment(
    std::string const& segment_name,
    uint64_t size_in_byte,
    bool enable_warning) {
  if (!check_memory_segment_exists(segment_name)) {
    auto new_named_memory_segment =
        std::make_shared<NamedSharedMemorySegment>(segment_name, size_in_byte);
    new_named_memory_segment->create_writeable();
    mNamedMemorySegments[segment_name] = new_named_memory_segment;
  } else {
    if (enable_warning) {
      Logger::LOG_WARNING << "Named Shared Memory \"" << segment_name
                          << "\" already exists!" << std::endl;
    }
  }
}

void NamedSharedMemoryController::add_read_only_memory_segment(
    std::string const& segment_name,
    bool enable_warning) {
  if (!check_memory_segment_exists(segment_name)) {
    auto new_named_memory_segment =
        std::make_shared<NamedSharedMemorySegment>(segment_name, 0);
