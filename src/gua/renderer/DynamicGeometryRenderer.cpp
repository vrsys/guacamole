...
DynamicGeometryRenderer::DynamicGeometryRenderer(RenderContext const &ctx, SubstitutionMap const &smap)
...
void DynamicGeometryRenderer::render(Pipeline &pipe, PipelinePassDescription const &desc)
{
    ...
    if(sorted_objects != scene.nodes.end() && sorted_objects->second.size() > 0)
    {
        ...
        std::sort(sorted_objects->second.begin(), sorted_objects->second.end(), [](node::Node *a, node::Node *b) {
            return reinterpret_cast<node::DynamicGeometryNode *>(a)->get_material()->get_shader() < reinterpret_cast<node::DynamicGeometryNode *>(b)->get_material()->get_shader();
        });
        ...
        {
            auto dynamic_geometry_node(reinterpret_cast<node::DynamicGeometryNode *>(object));
            if(pipe.current_viewstate().shadow_mode && dynamic_geometry_node->get_shadow_mode() == ShadowMode::OFF)
            {
                continue;
            }

            if(!dynamic_geometry_node->get_render_to_gbuffer())
            {
                continue;
            }
            ...
            if(!dynamic_geometry_node->get_render_volumetric())
            {
            ...
            else
            {
                ...
                if(dynamic_geometry_node->get_render_vertices_as_points())
                {
            ...
            if(current_material_shader != dynamic_geometry_node->get_material()->get_shader())
            {
                current_material_shader = dynamic_geometry_node->get_material()->get_shader();
                ...
                {
                    Logger::LOG_WARNING << "DynamicGeometryPass::process(): Cannot find material: " << dynamic_geometry_node->get_material()->get_shader_name() << std::endl;
                }
                //}
            ...
            if(current_shader_program && dynamic_geometry_node->get_geometry())
            {
                auto model_view_mat = scene.rendering_frustum.get_view() * dynamic_geometry_node->get_cached_world_transform();
                UniformValue normal_mat(math::mat4f(scm::math::transpose(scm::math::inverse(dynamic_geometry_node->get_cached_world_transform()))));

                int rendering_mode = pipe.current_viewstate().shadow_mode ? (dynamic_geometry_node->get_shadow_mode() == ShadowMode::HIGH_QUALITY ? 2 : 1) : 0;

                current_shader_program->apply_uniform(ctx, "gua_model_matrix", math::mat4f(dynamic_geometry_node->get_cached_world_transform()));
                ...
                if(rendering_mode != 1)
                {
                    dynamic_geometry_node->get_material()->apply_uniforms(ctx, current_shader_program.get(), view_id);
                }

                bool show_backfaces = dynamic_geometry_node->get_material()->get_show_back_faces();
                bool render_wireframe = dynamic_geometry_node->get_material()->get_render_wireframe();
                ...
                float screen_space_line_width = dynamic_geometry_node->get_screen_space_line_width();
                // float screen_space_point_size = dynamic_geometry_node->get_screen_space_point_size();
                ...
                dynamic_geometry_node->get_geometry()->draw(pipe.get_context());
            }
        }
