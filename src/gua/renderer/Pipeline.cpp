...
scm::gl::texture_2d_ptr Pipeline::render_scene(
  ...
  for (auto const& cam : camera.pre_render_cameras) {
    if (!context_.render_pipelines.count(cam.uuid)) {
      context_.render_pipelines.insert(
          std::make_pair(cam.uuid,
                         std::make_shared<Pipeline>(
                             context_, camera.config.get_resolution())));
    }
  ...
  if (reload_gbuffer) {
    ...
    math::vec2ui new_gbuf_size(std::max(1U, camera.config.resolution().x),
                               std::max(1U, camera.config.resolution().y));
...
void Pipeline::generate_shadow_map(node::LightNode& light,
  ...
  if (!shadow_map) {
    needs_redraw = true;
    for (auto it(shadow_map_res_->unused_shadow_maps.begin());
         it != shadow_map_res_->unused_shadow_maps.end();
         ++it) {
  ...
  if (!shadow_map) {
    shadow_map = std::make_shared<ShadowMap>(
        context_, math::vec2ui(map_width, viewport_size));
  }
  ...
  // set view parameters
  auto original_camera_resolution =
  ...
  switch (light.data.get_type()) {
    ...
    case node::LightNode::Type::POINT:
      generate_shadow_map_pointlight(
          shadow_map, light, light_block, viewport_size, needs_redraw);
      break;
    case node::LightNode::Type::SPOT:
      generate_shadow_map_spotlight(
          light, light_block, viewport_size, needs_redraw);
      break;
    default:
      throw std::runtime_error(
          "Pipeline::generate_shadow_map(): Lightnode type not supported.");
  }
...
void Pipeline::render_shadow_map(LightTable::LightBlock& light_block,
                                 Frustum const& frustum,
                                 unsigned cascade_id,
                                 unsigned viewport_size,
                                 bool redraw) {

  light_block.projection_view_mats[cascade_id] =
      math::mat4f(frustum.get_projection() * frustum.get_view());

  // only render shadow map if it hasn't been rendered before this frame
  if (redraw) {
    current_viewstate_.scene = current_viewstate_.graph->serialize(
                    frustum,
                    frustum,
                    math::get_translation(current_viewstate_.camera.transform),
...
void Pipeline::generate_shadow_map_sunlight(
  ...
  for (int cascade(0); cascade < splits.size() - 1; ++cascade) {
    ...
    math::mat4 screen_in_sun_space(scm::math::make_translation(
        center) * scm::math::make_scale(
                      radius_in_sun_space * 2, radius_in_sun_space * 2, 1.0f));
    auto sun_screen_transform(transform * screen_in_sun_space);

    // calculate transformation of shadow eye
    auto sun_eye_transform(
        scm::math::make_translation(sun_screen_transform.column(3)[0],
...
void Pipeline::generate_shadow_map_pointlight(
  ...
  std::vector<PipelineViewState::ViewDirection> view_directions = {
    PipelineViewState::front, PipelineViewState::back, PipelineViewState::top,
...
void Pipeline::bind_gbuffer_input(
  ...
  shader->set_uniform(
      context_, 1.0f / gbuffer_->get_width(), "gua_texel_width");
  shader->set_uniform(
      context_, 1.0f / gbuffer_->get_height(), "gua_texel_height");
...
void Pipeline::bind_camera_uniform_block(unsigned location) const {
  get_context().render_context
      ->bind_uniform_buffer(camera_block_.block().block_buffer(), location);
}
