...
PLODRessource::PLODRessource(pbr::model_t model_id, bool is_pickable)
    ...
    scm::gl::boxf bb = pbr::ren::ModelDatabase::GetInstance()->
                       GetModel(model_id)->kdn_tree()->bounding_boxes()[0];
...
void PLODRessource::draw(RenderContext const& ctx, 
    ...
    for (std::vector<pbr::ren::Cut::NodeSlotAggregate>::const_iterator k = node_list.begin(); 
            k != node_list.end(); ++k, ++node_counter) {
...
void PLODRessource::ray_test(Ray const& ray, PickResult::Options options,
    ...
    const auto* tree = pbr::ren::ModelDatabase::GetInstance()->
                       GetModel(model_id_)->kdn_tree();
    ...
    bool has_hit = false;
    PickResult pick = PickResult(0.f, owner, ray.origin_, 
        ...
        for (pbr::node_t k = 0; k < fan_factor; ++k) {
            ...
            if (hit_ray.t_max_ >= 0.f) {
                if (n >= first_leaf) {
                    ...
                    float dist = scm::math::length(owner_transform * 
                                                   hit_ray.origin_ - 
                                                   world_origin);
