    ...
    if (sorted_objects != pipe.get_scene().nodes.end() && sorted_objects->second.size() > 0) {
      ...
      for (auto const& object_list : sorted_objects->second) {
        ...
        if (material) {
          ...
          auto tri_mesh_node(reinterpret_cast<node::SkeletalAnimationNode*>(object_list.second[0]));
          ...
          for (unsigned current_bind(0); current_bind < rebind_num; ++current_bind) {
            ...
            for (int i(0); i < object_count; ++i) {
              ...
              auto const& node(reinterpret_cast<node::SkeletalAnimationNode*>(object_list.second[current_object]));
          ...
          Logger::LOG_WARNING << "SkeletalAnimationPass::process(): Cannot find material: " << object_list.first << std::endl;
        }
      }
