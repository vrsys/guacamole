...
// class header
#include <gua/renderer/DynamicLineRenderer.hpp>
...
DynamicLineRenderer::DynamicLineRenderer(RenderContext const &ctx, SubstitutionMap const &smap)
...
#error "This works only with GUACAMOLE_RUNTIME_PROGRAM_COMPILATION enabled"
    ...
    std::string v_shader = factory.read_shader_file("resources/shaders/dynamic_geometry_shader.vert");
    std::string f_shader = factory.read_shader_file("resources/shaders/dynamic_geometry_shader.frag");
...
void DynamicLineRenderer::render(Pipeline &pipe, PipelinePassDescription const &desc)
{
    auto &scene = *pipe.current_viewstate().scene;
    auto sorted_objects(scene.nodes.find(std::type_index(typeid(node::DynamicLineNode))));
    ...
    if(sorted_objects != scene.nodes.end() && sorted_objects->second.size() > 0)
    {
        ...
        std::sort(sorted_objects->second.begin(), sorted_objects->second.end(), [](node::Node *a, node::Node *b) {
            return reinterpret_cast<node::DynamicLineNode *>(a)->get_material()->get_shader() < reinterpret_cast<node::DynamicLineNode *>(b)->get_material()->get_shader();
        });
        ...
        {
            auto dynamic_line_node(reinterpret_cast<node::DynamicLineNode *>(object));
            if(pipe.current_viewstate().shadow_mode && dynamic_line_node->get_shadow_mode() == ShadowMode::OFF)
            {
                continue;
            }
            // std::cout<<"drawing a triangle"<<std::endl;
            if(!dynamic_line_node->get_render_to_gbuffer())
            {
                continue;
            }
            ...
            if(!dynamic_line_node->get_render_volumetric())
            {
                current_material_shader_map = &programs_;
                current_shader_stages = &program_stages_;
            }

            if(current_material_shader != dynamic_line_node->get_material()->get_shader())
            {
                current_material_shader = dynamic_line_node->get_material()->get_shader();
                if(current_material_shader)
                {
                    ...
                    else
                    {
                        ...
                        current_shader_program = std::make_shared<ShaderProgram>();
                         ...
                         bool early_fragment_test_enabled = dynamic_line_node->get_material()->get_enable_early_fragment_test();
...
#else
                        bool virtual_texturing_enabled = dynamic_line_node->get_material()->get_enable_virtual_texturing();
                ...
                {
                    Logger::LOG_WARNING << "DynamicLinePass::process(): Cannot find material: " << dynamic_line_node->get_material()->get_shader_name() << std::endl;
                }
            ...
            if(current_shader_program && dynamic_line_node->get_geometry())
            {
                auto model_view_mat = scene.rendering_frustum.get_view() * dynamic_line_node->get_cached_world_transform();
                UniformValue normal_mat(math::mat4f(scm::math::transpose(scm::math::inverse(dynamic_line_node->get_cached_world_transform()))));

                int rendering_mode = pipe.current_viewstate().shadow_mode ? (dynamic_line_node->get_shadow_mode() == ShadowMode::HIGH_QUALITY ? 2 : 1) : 0;

                current_shader_program->apply_uniform(ctx, "gua_model_matrix", math::mat4f(dynamic_line_node->get_cached_world_transform()));
                ...
                if(rendering_mode != 1)
                {
                    dynamic_line_node->get_material()->apply_uniforms(ctx, current_shader_program.get(), view_id);
                }

                // bool show_backfaces   = dynamic_line_node->get_material()->get_show_back_faces();
                bool render_wireframe = dynamic_line_node->get_material()->get_render_wireframe();
                ...
                float screen_space_line_width = dynamic_line_node->get_screen_space_line_width();
                // float screen_space_point_size = dynamic_line_node->get_screen_space_point_size();
                ...
                dynamic_line_node->get_geometry()->draw(pipe.get_context());
            }
        }
        target.unbind(ctx);
