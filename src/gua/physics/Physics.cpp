...
void Physics::set_gravity(math::vec3 const& gravity) {
    dw_->setGravity(math::vec3_to_btVector3(gravity));
}

////////////////////////////////////////////////////////////////////////////////

math::vec3 Physics::get_gravity() const {
    return math::btVector3_to_vec3(dw_->getGravity());
}
...
void Physics::simulation_rate_reduction(bool enabled, int max_sim_time) {
    ...
    reduce_sim_rate_ = enabled,
    max_sim_time_ = chrono::microseconds(max_sim_time);
}
...
void Physics::add_constraint(Constraint * ct,
                             bool disable_collisions_between_linked_bodies) {
    if (ct && ct->ct_ &&
        std::find(constraints_.begin(), constraints_.end(), ct) ==
            constraints_.end()) {
        lock_guard<mutex> lk(simulation_mutex_);
...
void Physics::simulate() {
    ...
    while (!is_stopped_.load()) {
...

        {
            ...
            std::for_each(constraints_.begin(),
                          constraints_.end(),
                          std::mem_fn(&Constraint::validate));
            //for (auto ct: constraints_) ct->validate();

            const btScalar current_timestep =
                chrono::duration_cast<chrono::microseconds>(
                    current_time - last_time).count() * 1.e-6f;

            dw_->stepSimulation(current_timestep,
                                physics_default_max_sub_steps,
                                fixed_timestep_);
            //Swap motion state transforms
            {
                lock_guard<SpinLock> lk(motion_state_update_mutex_);
        ...
        physics_fps_ =
            1.e6f /
            (chrono::duration_cast<chrono::microseconds>(
                chrono::high_resolution_clock::now() - current_time).count());
