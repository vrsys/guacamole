...
SliderConstraint::SliderConstraint(RigidBodyNode* body_a,
                                   const math::mat4& frame_a)
    : Constraint(body_a, nullptr),
      frame_a_(frame_a, false),
      frame_b_(math::mat4::identity(), false) {
  auto ct = new btSliderConstraint(*(body_a->get_bullet_rigid_body()),
...
void SliderConstraint::set_angular_limit(float lower, float upper) {
  if (lower_angular_limit_.first == lower &&
      upper_angular_limit_.first == upper)
    return;
  lock_guard<SpinLock> lk(lock_);
