...
HingeConstraint::HingeConstraint(RigidBodyNode* body_a,
                                 const math::vec3& pivot_a,
                                 const math::vec3& axis_a)
    : Constraint(body_a, nullptr),
      pivot_a_(pivot_a, false),
      pivot_b_(math::vec3(0.f), false),
      axis_a_(axis_a, false),
      axis_b_(math::vec3(0.f), false) {
...
HingeConstraint::HingeConstraint(RigidBodyNode* body_a,
    ...
    : Constraint(body_a, body_b),
      pivot_a_(pivot_a, false),
      pivot_b_(pivot_b, false),
      axis_a_(axis_a, false),
      axis_b_(axis_b, false) {
  auto ct = new btHingeConstraint(*(body_a->get_bullet_rigid_body()),
...
void HingeConstraint::enable_angular_motor(bool enable,
                                           float target_velocity,
                                           float max_impulse) {
  if (motor_enabled_.first == enable &&
      motor_target_velocity_.first == target_velocity &&
      motor_max_impulse_.first == max_impulse)
    return;
  lock_guard<SpinLock> lk(lock_);
...
bool HingeConstraint::motor_enabled() const {
  return motor_enabled_.first;
}

////////////////////////////////////////////////////////////////////////////////

float HingeConstraint::motor_target_velocity() const {
  return motor_target_velocity_.first;
}

////////////////////////////////////////////////////////////////////////////////

float HingeConstraint::motor_target_impulse() const {
  return motor_max_impulse_.first;
}
...
/* virtual */ void HingeConstraint::update_constraint() {
  ...
  if (axis_a_.second || axis_b_.second || pivot_a_.second || pivot_b_.second) {
    ...
    frame_a.getBasis().setValue(rbAxisA1.getX(),
                                rbAxisA2.getX(),
                                axisInA.getX(),
                                rbAxisA1.getY(),
                                rbAxisA2.getY(),
    ...
    frame_b.getBasis().setValue(rbAxisB1.getX(),
                                rbAxisB2.getX(),
                                axisInB.getX(),
                                rbAxisB1.getY(),
                                rbAxisB2.getY(),
  ...
  if (motor_enabled_.second || motor_target_velocity_.second ||
      motor_max_impulse_.second) {
    ct->enableAngularMotor(motor_enabled_.first,
                           motor_target_velocity_.first,
                           motor_max_impulse_.first);
