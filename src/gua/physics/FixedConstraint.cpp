...
FixedConstraint::FixedConstraint(RigidBodyNode* body_a,
                                 const math::mat4& frame_a,
                                 bool lock_rotation,
                                 bool lock_translation)
    : Constraint(body_a, nullptr),
      frame_a_(frame_a, false),
      frame_b_(math::mat4::identity(), false),
      lock_rotation_(lock_rotation, false),
      lock_translation_(lock_translation, false) {
  auto ct = new btGeneric6DofConstraint(*(body_a->get_bullet_rigid_body()),
                                        math::mat4_to_btTransform(frame_a),
                                        true);
...
FixedConstraint::FixedConstraint(RigidBodyNode* body_a,
                                 ...
                                 const math::mat4& frame_a,
                                 const math::mat4& frame_b,
                                 bool lock_rotation,
                                 bool lock_translation)
    : Constraint(body_a, body_b),
      frame_a_(frame_a, false),
      frame_b_(frame_b, false),
      lock_rotation_(lock_rotation, false),
      lock_translation_(lock_translation, false) {
  auto ct = new btGeneric6DofConstraint(*(body_a->get_bullet_rigid_body()),
                                        *(body_b->get_bullet_rigid_body()),
                                        math::mat4_to_btTransform(frame_a),
                                        math::mat4_to_btTransform(frame_b),
                                        false);
  breaking_impulse_threshold_.first = ct->getBreakingImpulseThreshold();
  ct->setLinearUpperLimit(btVector3(0.f, 0.f, 0.f));
  ct->setAngularUpperLimit(btVector3(0.f, 0.f, 0.f));
...
/* virtual */ void FixedConstraint::update_constraint() {
  ...
  if (frame_a_.second || frame_b_.second) {
    ct->setFrames(math::mat4_to_btTransform(frame_a_.first),
                  math::mat4_to_btTransform(frame_b_.first));
